'''
中序遍历的整体顺序为 左-中-右
可以这样想，（1）如果一个节点有左子树，那么这个节点的前驱节点为 左子树中最右的节点（左中右是一个整体，有左之后还是先输出左子树整体的最右节点）
          （2）如果一个节点没有左子树，则往上寻找，当父节点的右孩子等于当前节点时，父节点即为当前节点的前驱节点
'''
class Node:
    left = None
    right = None
...
def getSuccessorNode(node):
    ...
    if node.left != None:
        return getRightMost(node.left)#找到某棵树的最左节点
    else:
        parent = node.parent
